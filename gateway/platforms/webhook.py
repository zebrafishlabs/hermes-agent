...
# ---------------------------------------------------------------------------
# Plugin route registry (carried fork hook, ESC-286)
# ---------------------------------------------------------------------------
...
# is instantiated.  Requests delegate here at dispatch time (see
# ``_resolve_route``), making delegation independent of adapter connect order.
#
...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def _resolve_route(self, request: "web.Request") -> "tuple[str, Optional[dict], Any, Optional[web.Response]]":
        ...
        # Order-independent plugin-route delegation (carried fork hook,
        # ESC-286): a platform plugin (e.g. Linear, Sentry) owns specific
        # paths under /webhooks/ via _plugin_route_registry. Checked at
        # request time so delegation is independent of adapter connect order.
    ...
    async def _handle_webhook(self, request: "web.Request") -> "web.Response":
        ...
        # Order-independent plugin-route delegation (carried fork hook,
        # ESC-286): platform plugins (Linear, Sentry) own specific paths under
        # /webhooks/ via _plugin_route_registry. Checked at request time so
        # delegation is independent of adapter connect order.
        plugin_handler = _plugin_route_registry.get(
            f"/webhooks/{request.match_info.get('route_name', '')}"
        )
