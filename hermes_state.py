...
class SessionDB(
    ...
    def close_session(self, session_id: str) -> None:
        ...
        Carried fork method (ESC-272 /close) — upstream has no close_session;
        the mixins in hermes_state_*.py own the rest of the session-lifecycle
        surface. Sets ``closed = 1`` and backfills ``ended_at`` /
        ``end_reason`` only if they were not already populated (so an earlier
        end record wins).
        """
        def _do(conn):
            conn.execute(
